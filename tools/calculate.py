...
def video_label_length(dataset='shanghaitech'):
    if dataset == 'shanghaitech':
        label_path = "./datasets/shanghaitech/frame_masks/"
        video_length = {}
        ...
        test_frame_path = './datasets/' + dataset + '/testing/'
...
def load_objects(dataset, frame_num=7):
    ...
    detect_dir = f'detect/{dataset}_test_detect_result.pkl'
...
def remake_video_3d_output(video_output, dataset='ped2', frame_num=7):
    ...
    for i in range(len(video_l)):
        ...
        for i in range(video_length[video]):
            frame_scores[i] = video_[:, :, i].min()

        frame_scores -= frame_scores.min()
        frame_scores /= frame_scores.max()
        
        return_output.append(frame_scores)
...
def remake_video_output(video_output, dataset='ped2'):
    ...
    for i in range(len(video_l)):
        ...
        for fno in frame_l:
            ...
            video_[fno] = object_record.min(0)
...
def evaluate_auc(video_output, dataset='shanghaitech'):
    ...
    smoothed_results, aver_smoothed_result = eval.evaluate_all(result_dict, reverse=True, smoothing=True)
...
if __name__ == '__main__':
    ...
    if args.dataset == 'shanghaitech':
        video_output = remake_video_output(output, dataset=args.dataset)
    else:
        video_output = remake_video_3d_output(output, dataset=args.dataset, frame_num=args.frame_num)


    evaluate_auc(video_output,  dataset=args.dataset)
